...
The client is built lazily (on first use, not at import time) so the
app can still start up even before S3/storage credentials are
configured -- useful early on, since upload is only one of several
features.
"""
import uuid
...
def _get_s3_client():
    return boto3.client(
        "s3",
        endpoint_url=settings.s3_endpoint_url or None,  # "" from an unset env var must become None, not passed through
...
def upload_file_to_storage(*, tenant_id: UUID, filename: str, contents: bytes) -> str:
    ...
    _get_s3_client().put_object(Bucket=settings.s3_bucket, Key=key, Body=contents)
    return key
